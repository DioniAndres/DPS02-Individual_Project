...
categorical_columns = ['original_language', 'production_countries', 'status']
...
# %%
# Irrelevant Columns are remove
Columnas_Irrelevantes = ['tagline', 'collection_id', 'collection_name', 'backdrop_path', 'company_names', 'company_ids']
df_movies.drop(columns=Columnas_Irrelevantes, inplace=True)
...
# 'Most Common Job Titles based on Genre'

def extract_job_titles(crew_data):
    try:
        crew_data = ast.literal_eval(crew_data)
        job_titles = [member['job'] for member in crew_data if 'job' in member]
        return job_titles
    except:
        return []

df_movies['job_titles'] = df_movies['crew'].apply(extract_job_titles)

df_movies_unnested = df_movies.explode('genre_names')

df_movies_unnested.dropna(subset=['job_titles', 'genre_names'], inplace=True)

df_movies_exploded = df_movies_unnested.explode('job_titles')

common_job_titles = df_movies_exploded.groupby(['genre_names', 'job_titles']).size().reset_index(name='count')

def get_top_job_titles(group):
    try:
        return group.nlargest(5, 'count')
    except:
        return group

top_job_titles_by_genre = common_job_titles.groupby('genre_names').apply(get_top_job_titles)

# Step 7: Bar plot of the most common job titles by genre
plt.figure(figsize=(12, 6))
sns.barplot(data=top_job_titles_by_genre, x='job_titles', y='count', hue='genre_names', palette='viridis')
plt.title('Most Common Job Titles based on Genre')
