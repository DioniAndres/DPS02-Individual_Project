...
# Load the merged DataFrame from an Excel file
...
def franchise(franchise_name: str):
    try:
        ...
        total_revenue = franchise_info['revenue'].sum()
        average_revenue = franchise_info['revenue'].mean()
...
def recommendation(title: str):
    try:
        # Ensure that the movie title is in lowercase for case-insensitive matching
        title = title.lower()
        ...
        # Get indices of top 5 similar movies (excluding the input movie itself)
        similar_indices = cosine_similarities.argsort()[:-6:-1]

        # Get movie titles of the top 5 similar movies
